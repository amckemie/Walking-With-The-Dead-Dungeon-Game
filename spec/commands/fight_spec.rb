...
describe WWTD::Fight do
  ...
  describe 'checking if should fight' do
    it 'checks a players room and if there is a zombie in that room for the player. Returns "no zombie to fight here" if not' do
      ...
      expect(result.message).to eq("Save your energy. There's no zombie to fight here.")
    end
  end

  describe 'winning fight' do
    before(:each) do
      @result = subject.run(@player_1, @zombie, false)
    end

    xit "returns a success message that the person won the fight if they have more strength than opponent" do
      expect(@result.success?).to eq(true)
      expect(@result.message).to eq("Whew! You killed the bloody clown zombie!")
    end

    xit "deletes the opponent from the players game if they are killed when the person wins" do
      expect(@result.deleted?).to eq(true)
    end

    xit 'decreases the players strength by the opponents strength and saves it in the db' do
  ...
  describe 'losing fight' do
    before(:each) do
      ...
      @result2 = subject.run(@player2, @zombie2, false)
    end

    xit "returns that the person lost the fight if they have less than or equal strength than/of opponent" do
      expect(@result2.message).to eq('Oh nooossssssss! The zombie bit you! So sorry, but you are now infected.')
    end

    xit "kills the person if they lose the fight" do
  ...
  describe 'shoot' do
    xit 'randomly chooses if the player wins if they "shoot" the opponent' do
      shoot_result = subject.run(@player_1, @zombie, true)
